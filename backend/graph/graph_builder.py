...
def build_viz_payload(
    ...
    blocked_titles_lower = {
        ...
        for m in state["blocked_movies"]
    }
    soft_disliked_count = sum(
        1 for t in state.get("disliked_movies", [])
        if isinstance(t, str) and t.lower() not in blocked_titles_lower
    )
    total_blocked = len(state["blocked_movies"]) + soft_disliked_count
    return {
        "nodes": nodes,
        "edges": edges,
