...
class PreferenceGraph:
    ...
    def to_viz_payload(self) -> dict:
        ...
        # Also show soft-disliked movies as red nodes (they are filtered from recommendations)
        blocked_titles_lower = {
            (m.get("title") if isinstance(m, dict) else str(m)).lower()
            for m in self.state["blocked_movies"]
        }
        for title in self.state.get("disliked_movies", []):
            if isinstance(title, str) and title.lower() not in blocked_titles_lower:
                n = nid("disliked", title)
                if n not in seen:
                    nodes.append({"id": n, "label": title, "type": "movie_blocked", "weight": 0.0})
                    seen.add(n)
                    edges.append({"source": "user", "target": n, "weight": 0.0, "type": "blocked"})
