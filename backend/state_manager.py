...
class StateManager:
    ...
    def trigger_session_end(self, mode: str) -> dict:
        ...
        if mode == "commit":
            ...
            # Collect titles the user explicitly disliked during this session
            session_disliked_titles = {
                it["title"].lower()
                for it in self.session_graph.state.get("interactions", [])
                if it.get("type") == "disliked"
            }
            # Also respect any soft dislikes already in the preference graph
            perm_disliked_titles = {
                t.lower() for t in self.preference_graph.state.get("disliked_movies", [])
            }
            perm_blocked_titles = {
                (m.get("title") if isinstance(m, dict) else str(m)).lower()
                for m in self.preference_graph.state.get("blocked_movies", [])
            }
            skip_titles = session_disliked_titles | perm_disliked_titles | perm_blocked_titles
            ...
            for it in self.session_graph.state.get("interactions", []):
                ...
                if float(it.get("weight", 0.0)) <= 0:
                    continue
                # Skip movies the user explicitly disliked or soft-disliked
                if it.get("title", "").lower() in skip_titles:
                    continue
